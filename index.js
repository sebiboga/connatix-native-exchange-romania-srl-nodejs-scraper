...
async function fetchGreenhouseJobs() {
  ...
  console.log(`Fetching Greenhouse jobs from: ${url}`);
...
function parseGreenhouseJobs(apiData) {
  const jobs = apiData.jobs || [];
  const total = apiData.meta?.total || jobs.length;

  return {
    jobs: jobs.map(job => {
      const location = job.location?.name || "";
      const workmode = location.toLowerCase().includes("remote") ? "remote" : "on-site";
      ...
      return {
        ...
        tags: (job.departments || []).map(d => d.name.toLowerCase()),
        company_name: job.company_name || "JWX"
      };
    }),
    total
  };
...
async function scrapeAllListings(testOnlyOnePage = false) {
  ...
  const data = await fetchGreenhouseJobs();
  const result = parseGreenhouseJobs(data);
...
async function main() {
  ...
  try {
    ...
    console.log(`Jobs scraped from JWX Careers website: ${scrapedCount}`);
    ...
    console.log(`Jobs scraped from JWX website: ${scrapedCount}`);
...
export { parseGreenhouseJobs, mapToJobModel, transformJobsForSOLR };
