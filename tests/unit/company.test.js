...
const CONNATIX_ANAF_RECORD = {
  cui: 35861771,
  name: 'CONNATIX NATIVE EXCHANGE ROMANIA SRL',
  address: 'PLOIEŞTI, 36-36N, 38, Municipiul Cluj-Napoca, Cluj',
  caenCode: '6201',
  inactive: false,
  vatRegistered: true,
...
describe('company.js', () => {
  ...
  describe('getCompanyData (no cache)', () => {
    it('should fetch CONNATIX via direct CIF lookup and return company data', async () => {
      mockFetch.mockResolvedValueOnce(anafCompanyResponse(CONNATIX_ANAF_RECORD));

      const result = await company.getCompanyData();

      expect(result).toHaveProperty('company', 'CONNATIX NATIVE EXCHANGE ROMANIA SRL');
      expect(result).toHaveProperty('cif', '35861771');
      expect(result).toHaveProperty('active', true);
      expect(result).toHaveProperty('anafData');
      expect(result.anafData.name).toBe('CONNATIX NATIVE EXCHANGE ROMANIA SRL');
    });
    ...
    it('should throw when ANAF returns no company name', async () => {
      mockFetch.mockResolvedValueOnce(anafCompanyResponse({ cui: 35861771, name: null }));
  ...
  describe('getCompanyData (with cache)', () => {
    ...
    it('should use cached company data when available', async () => {
      ...
      expect(result.cif).toBe('35861771');
  ...
  describe('validateAndGetCompany', () => {
    ...
    it('should return company data with status active', async () => {
      mockFetch
        .mockResolvedValueOnce(anafCompanyResponse(CONNATIX_ANAF_RECORD))
        .mockResolvedValueOnce(solrResponse(5, [
          { url: 'https://test.com/1', title: 'Job 1' },
          { url: 'https://test.com/2', title: 'Job 2' }
        ]))
        .mockResolvedValueOnce(peviitorResponse([{ company: 'CONNATIX NATIVE EXCHANGE ROMANIA SRL' }]));
    ...
    if (CONNATIX_ANAF_RECORD.inactive) {
      it('should return inactive status when company is inactive', async () => {
        const inactiveRecord = { ...CONNATIX_ANAF_RECORD, inactive: true };
