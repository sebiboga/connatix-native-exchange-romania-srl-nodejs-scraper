...
describe('Integration: API Workflow', () => {
  ...
  describe('ANAF API', () => {
    ...
    it('should search for CONNATIX brand and find the company', async () => {
      ...
      const connatix = results.find(c =>
        c.name.toUpperCase().includes('CONNATIX') && c.statusLabel === 'Funcțiune'
      );
      expect(connatix).toBeDefined();
      expect(connatix.cui.toString()).toBe(COMPANY_CIF);
    }, 15000);
    ...
    it('should fetch company details by valid CIF', async () => {
      ...
      expect(data.cui).toBe(35861771);
      expect(data.name).toBe('CONNATIX NATIVE EXCHANGE ROMANIA SRL');
    ...
    it('should use cached data when API fails (getCompanyFromANAFWithFallback)', async () => {
      const cached = { cui: 35861771, name: 'CONNATIX NATIVE EXCHANGE ROMANIA SRL' };
  ...
  describe('SOLR Company Core', () => {
    ...
    itIfSolr('should query company core by ID', async () => {
      ...
      expect(company.company).toBe('CONNATIX NATIVE EXCHANGE ROMANIA SRL');
      expect(company.brand).toBe('connatix');
    ...
    itIfSolr('should have required company model fields', async () => {
      ...
      expect(company).toHaveProperty('brand', 'connatix');
  ...
  describe('SOLR Jobs Core', () => {
    ...
    itIfSolr('should query jobs by CIF and return valid data', async () => {
      ...
      expect(job).toHaveProperty('company', 'CONNATIX NATIVE EXCHANGE ROMANIA SRL');
  ...
  describe('Full Validation Workflow', () => {
    ...
    it('should complete the ANAF → Peviitor validation path', async () => {
      const searchResults = await anaf.searchCompany('CONNATIX');
      expect(searchResults.length).toBeGreaterThan(0);

      const connatixCompany = searchResults.find(c =>
        c.name.toUpperCase().includes('CONNATIX') && c.statusLabel === 'Funcțiune'
      );
      expect(connatixCompany).toBeDefined();

      const anafData = await anaf.getCompanyFromANAF(connatixCompany.cui.toString());
      expect(anafData.name).toBe('CONNATIX NATIVE EXCHANGE ROMANIA SRL');
    ...
    itIfSolr('should have matching CIF in company core', async () => {
      ...
      expect(solrResult.docs[0].company).toBe('CONNATIX NATIVE EXCHANGE ROMANIA SRL');
    }, 30000);
    ...
    itIfSolr('should validate company and query SOLR for existing jobs', async () => {
      ...
      expect(companyResult.company).toBe('CONNATIX NATIVE EXCHANGE ROMANIA SRL');
